...
const	 unsigned char auchCRCLo[256] =
    ...
    unsigned char e=0;
	extern unsigned char Crc_leng;
//************************************************************
//函数名：cal_crc(unsigned char *puchMsg, unsigned char usDataLen) 
//输入：需要生成CRC校验码的字符串首地址，字符串的字节个数
//输出：16位的CRC校验码
//功能描述：生成CRC校验码
unsigned int cal_crc(unsigned char *puchMsg, unsigned char usDataLen) 
{
	uchCRCHi = 0xFF ;	
	uchCRCLo = 0xFF ;	
	while (usDataLen--)		
