...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagAuto)
    {
        info->event[info->num] = EVENT_Auto;
        info->num++;
        dataPoints->valueAuto = gizStandardDecompressionValue(Auto_BYTEOFFSET,Auto_BITOFFSET,Auto_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flaglamp)
    {
        info->event[info->num] = EVENT_lamp;
        info->num++;
        dataPoints->valuelamp = gizStandardDecompressionValue(lamp_BYTEOFFSET,lamp_BITOFFSET,lamp_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueAuto != cur->valueAuto)
    {
        //GIZWITS_LOG("valueAuto Changed\n");
        ret = 1;
    }
    if(last->valuelamp != cur->valuelamp)
    {
        //GIZWITS_LOG("valuelamp Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Auto_BYTEOFFSET,Auto_BITOFFSET,Auto_LEN,(uint8_t *)devStatusPtr,dataPoints->valueAuto);
    gizStandardCompressValue(lamp_BYTEOFFSET,lamp_BITOFFSET,lamp_LEN,(uint8_t *)devStatusPtr,dataPoints->valuelamp);
